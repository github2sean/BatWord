...
def handDocs(codePath, descPath):
    ...
    codePath = '/Users/sean/Downloads/java/1  Java  线上线下相结合的快递包装回收综合管理系统.docx'
    # if codePath.endswith(doc):
    # codePath = changeFileFormat(codePath)
    descPath = '/Users/sean/Downloads/java/线上线下相结合的快递包装回收综合管理系统.docx'
    ...
    document.save("/Users/sean/Downloads/java/1  Java  线上线下相结合的快递包装回收综合管理系统_" + getNowTime() + ".docx")
    return
...
def findUsefulFiled(path):
    ...
    for p in infoDocument.paragraphs:
        content = p.text
        content = content.replace('详情如下图所示：', '').replace('点击', '')
        if content == ' 。' or content == ' ，' or content == ' ：' or content == ' 。' or content == ' ！' or content == '？':
            continue
        if p.style.name == 'Heading 2':
            ...
            for r in p.runs:
                if True and len(r.text) > 44:
                    functionList.append(r.text.replace('详情如下图所示：', '').replace('点击', ''))
    print(functionList)
    print(classList)
    ...
    if descList:
        ...
        for row, up in enumerate(handDesc(listVolume, lineDesc)):
            ...
            if row == 0:
                ...
                p.insert_paragraph_before('* ' + up, p.style).paragraph_format.left_indent = Pt(20)
        p.insert_paragraph_before('*/ ').paragraph_format.left_indent = Pt(20)
...
# def changeFileFormat(path):
#     word = wc.Dispatch("Word.Application")
#     doc = word.Documents.Open(path)
#     name = path[:path.index('.')]
#     doc.SaveAs(name + '.docx', 12)
#     doc.Close()
#     word.Quit()
#     return name + '.docx'
